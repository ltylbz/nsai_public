 ...
 * @NApiVersion 2.1
 *
 * LLM gateway for Search Insight — resolves provider/model from shared
 * custom records, dispatches chat completion via N/llm or HTTPS, and
 * returns parsed text used to narrate the saved-search analysis.
 */
define([
  "N/https",
  "N/search",
  ...
  const callLLM = (messages, options) => {
    ...
    if (provider.provider.toLowerCase() === "openrouter") {
      headers["HTTP-Referer"] =
        opts.httpReferer || "https://netsuite.local/search_insight";
      headers["X-Title"] = opts.title || "Search Insight";
    }
